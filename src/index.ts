 ...
 * - get_crate_dependencies: dependencies of a specific crate version
 * - get_crate_reverse_deps: crates that depend on a given crate
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Crates.io');
}
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        version: { type: 'string', description: 'Exact version (e.g. "1.40.0"), or omit for the latest stable.' },
      },
      required: ['name'],
    },
  },
  {
    name: 'get_crate_reverse_deps',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        limit: { type: 'number', description: 'Number of top dependents to return (default 20, max 100).' },
      },
      required: ['name'],
    },
  },
...
async function cratesGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw await httpError(res, 'crates.io API error');
  }
  return res.json();
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'get_crate_dependencies':
      return getCrateDependencies(args.name as string, (args.version as string) ?? '');
    case 'get_crate_reverse_deps':
      return getCrateReverseDeps(args.name as string, (args.limit as number) ?? 20);
    default:
      throw new Error(`Unknown tool: ${name}`);
  }
}

async function getCrateDependencies(name: string, version: string) {
  let ver = String(version ?? '').trim();
  if (!ver || ver === 'latest') {
    const meta = (await cratesGet(`/crates/${encodeURIComponent(name)}`)) as {
      crate: { max_stable_version: string | null; newest_version: string };
    };
    ver = meta.crate.max_stable_version ?? meta.crate.newest_version;
  }
  const data = (await cratesGet(
    `/crates/${encodeURIComponent(name)}/${encodeURIComponent(ver)}/dependencies`,
  )) as {
    dependencies: {
      crate_id: string;
      req: string;
...
async function getCrateReverseDeps(name: string, limit: number) {
  const perPage = Math.min(100, Math.max(1, limit));
  const data = (await cratesGet(
    `/crates/${encodeURIComponent(name)}/reverse_dependencies?per_page=${perPage}`,
  )) as {
    dependencies: { crate_id: string; req: string; kind: string; downloads: number }[];
    versions: { crate: string; num: string }[];
    meta: { total: number };
  };

  // crates.io returns parallel arrays: dependencies[i] pairs with versions[i];
  // the DEPENDENT crate is versions[i].crate, sorted by its downloads desc.
  const deps = data.dependencies ?? [];
  const vers = data.versions ?? [];
  return {
    crate: name,
    total_dependents: data.meta?.total ?? null,
    count: deps.length,
    dependents: deps.map((d, i) => ({
      crate: vers[i]?.crate ?? d.crate_id,
      version: vers[i]?.num ?? null,
      depends_on_req: d.req,
      kind: d.kind,
